...
build_plots <- function(words.table,metric="net"){
  ...
  if(metric=="nd"){
    ...
    p2 <- ggplot(data=words.table, aes(x=words.table[,1], y=words.table[,4]),environment=.e) + geom_point() + geom_smooth() + labs(title="Net additions per day, excluding outliers",x="Date",y="Word count")
    p1 <- qplot(words.table[,4],geom="blank",main="Net additions per day, excluding outliers",xlab="Word count",environment=.e) +
      geom_line(aes(y = ..density..,colour = "Observed"), stat="density") +
      geom_histogram(aes(y = ..density..), alpha=0.4) +
      scale_colour_manual(name = 'Density', values = c('red', 'blue')) +
      theme(legend.position="none")
    p4 <- ggplot(data=words.table, aes(x=words.table[,1], y=words.table[,5]),environment=.e) + geom_point() + geom_smooth() + labs(title="Net modifications per day, excluding outliers",x="Date",y="Word count")
    p3 <- qplot(words.table[,5],geom="blank",main="Net modifications per day, excluding outliers",xlab="Word count",environment=.e) +
      geom_line(aes(y = ..density..,colour = "Observed"), stat="density") +
      geom_histogram(aes(y = ..density..), alpha=0.4) +
      scale_colour_manual(name = 'Density', values = c('red', 'blue')) +
      theme(legend.position="none")
